...
#sub EDITOR_ACTION {
#	my ($iniref,$toxml,$SITE,$cgiv) = @_;
#
#	my $ACTION = $iniref->{'ACTION'};
#	my $MSG = '';
#	if ((not $iniref->{'_PREVIEW'}) && (defined $cgiv)) {
##		if ($ACTION eq 'EMAIL_TEST') {
##			# print STDERR 'CGIV/INIREF: '.Dumper($cgiv,$iniref,$SITE);
##			require TOXML::EMAIL;
##			my ($output,$warningsref) = &TOXML::EMAIL::sendmail($SITE->username(),$iniref->{'MAIL'},$cgiv->{'sendto'},PROFILE=>$SITE->profile(),TEST=>1);
##			if (not defined $warningsref) {
##				$MSG = "Sent email $iniref->{'MAIL'} to $cgiv->{'sendto'} at ".&ZTOOLKIT::pretty_date(time(),1)."<br>";
##				}
##			else {
##				$MSG = "Could not send email:<br>".join("<br>\n",@{$warningsref});
##				}
##			}
#		}
#	else {
...
#
#	my $out = '';
#	$out = qq~<div style="margin: 1px 0px 1px 0px;" id="editorDiv-$ID"><table width="100%" class="zoovytable"><tr><td>$MSG<form>~;
#	if ($ACTION eq 'EMAIL_TEST') {
#		}
#	else {
#		$out = "<font color='red'>Unknown EDITOR_ACTION ACTION=[$iniref->{'ACTION'}]</font>";
#		}
#	$out .= qq~</form></td></tr></table></div>~;
#
#
#	return('EDITOR_ACTION','',$out);
#	}
#
...
#
#	# print STDERR Dumper($SITE,$iniref);
#
#	my $SRC = $iniref->{'SRC'};
#
...
# 	my $SRC = ''; my $SRCtxt = '';
#	if ($iniref->{'SRC'} ne '') {
#		## the SRC is hardcoded, we'll use that.
#		$SRC = $iniref->{'SRC'};
#		}
#
#	## some prodlists allow the user to select from a source or specify a list.
#	##		if we're on one of those, the key/values are semi URI encoded + stored in the location $iniref->{'DATA'}
#	if ($iniref->{'DATA'} ne '') {
...
#		if (defined $params->{'SRC'}) { $SRC = $params->{'SRC'}; $SRCtxt = 'from ref in data location'; }
#		}
#
#	my ($ncats) = NAVCAT->new($SITE->username(),PRT=>$SITE->prt());
#	if (defined $ncats) { ($pretty,undef,$products) = $ncats->get($SITE->pageid()); $SRCtxt = ' from default navcat'; }
#	if ($SITE->pid() ne '') { $pretty = $SITE->pid(); }
#	if (($SRC eq '') && ($SITE->pageid() eq 'homepage')) { $SRC = 'NAVCAT:.';  }
#
#	if ($SRC eq '') {
#		## use default prodlist associated with navcat/page (that we already loaded)
#		$SRC = 'NAVCAT:'.$SITE->pageid();	# HINT: (we do this so we'll know where to save!)
#		}	
#	elsif (($SRC =~ /^NAVCAT\:(.*?)$/) || ($SRC =~ /^LIST\:(.*?)$/)) {
#		## loads products from an alternate category or list
#		$SRCtxt = 'from absolute navcat/list';
#		}
#	else {
#		$SRC = 'SREF('.$SRC.'):'.$SITE->siteserialize();
#		}
#	# $pretty .= " iniSRC[$iniref->{'SRC'}] SRC[$SRC] PG[$SITE->pageid()] $SRCtxt ".Dumper($iniref);
#	
#	print STDERR "PRODFINDER SRC: $SRC\n";
##	my ($html) = PRODUCT::FINDER::buildMain($SITE->username(),$SITE->prt(),'',{ src=>$SRC, div=>'' });
#
##	my $PID = '';
##	my $SRC = '';
##	$SRC = '.';
##
##	my $html .= qq~
##<input type="button" value=" Product Accessories " onClick="myControl.ext.admin.action.showFinderInModal('$SRC','$PID'); return false;">
##~;	
#
#	return('LISTEDITOR','List Editor',$html);
#	}
#
#
