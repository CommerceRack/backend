	...
	if ($v->{'_cmd'} eq 'adminProductCreate') {
		...
		if (not &JSONAPI::hadError(\%R)) {
			...
			foreach my $cmdset (@CMDS) {
				...
				#	}
				## INV-CONSTANT
				elsif ($VERB =~ /^INV-(PREORDER|SUPPLIER|RETURN|BACKORDER|CANCEL|ERROR|PICK|SIMPLE|WMS|MARKET|CONSTANT|BASETYPE)-(UUID|SKU|PID)-(INIT|SET|ANNOTATE|PREFERENCE|ADD|SUB|NUKE)$/) {
...
* GLOBAL/FLEXEDIT-FIELD-SET?attrib=xyz&json=
