...
{
	my ($lockid) = @_;
	# print STDERR "DEBUG STATEMENT - release lock id is currently [$lockid]\n";
	...
	if ($RESULT == $$) { 
		...
		print STDERR "Cannot remove $file because we are not the owner! [$RESULT] != [$$]\n";
		}
