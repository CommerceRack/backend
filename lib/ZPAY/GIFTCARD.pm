...
sub unified {
	...
	elsif ($payrec->{'tender'} ne 'GIFTCARD') {
	...
	if (defined $RESULT) {
		...
		if (not defined $txn) {
			...
			my $NOTE = sprintf("Giftcard %s [#%d]",&GIFTCARD::obfuscateCode($gcref->{'CODE'},0),$GCID);
			$payrec->{'txn'} = "$gcref->{'CODE'}.$txn";
			$payrec->{'note'} = $NOTE;
			$payrec->{'acct'} = &ZPAY::packit(
				{'GI'=>$GCID,'GC'=>$gcref->{'CODE'}
				});
			$O2->add_history("Giftcard ".&GIFTCARD::obfuscateCode($gcref->{'CODE'},0)." card_debit:$AMT card_balance_remain:$card_balance_remain",luser=>'*GIFTCARD');
			}
		}
