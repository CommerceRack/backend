...
#	$count = int($count);
#	if ($count==0) {
#		ZOOVY::confess($userref->{'USERNAME'},"Don't call AMAZON3::get_docs without passing a doc count!");
#		}
#
#	my @RDOCS = ();
#	my ($udbh) = &DBINFO::db_user_connect($userref->{'USERNAME'});	
#	my $xmlparser = new XML::Parser(Style=>'EasyTree');
#	$XML::Parser::EasyTree::Noempty=1;
#
#	my $pstmt = "select DOCTYPE,DOCID,ATTEMPTS,CREATED_GMT from AMAZON_DOCS where CREATED_GMT<".(time()-15)." and RETRIEVED_GMT=0 and ".
#		" MID=$userref->{'MID'} and PRT=$userref->{'PRT'} ".
#		" order by attempts,created_gmt limit 0,$count";
#	print STDERR $pstmt."\n";
#	my $sth = $udbh->prepare($pstmt);
#	$sth->execute();
#	while ( my ($doctype,$docid,$attempts,$created) = $sth->fetchrow() ) {
#		my $ERROR = undef;
#		my $xml = undef;
#		# next if ($doctype eq '_POST_ORDER_ACKNOWLEDGEMENT_DATA_');
#		# next if ($doctype eq '_POST_PRODUCT_PRICING_DATA_');
#		# next if ($doctype eq '_POST_ORDER_FULFILLMENT_DATA_');
#		# next if ($doctype eq '_POST_PRODUCT_IMAGE_DATA_');
#
#		my ($rdocid,$status) = &AMAZON3::getDocumentPS($userref,$docid);
#		my ($path) = &ZOOVY::resolve_userpath($userref->{'USERNAME'});
#		print STDERR "\n$userref->{'USERNAME'} $doctype $status\n$path/PRIVATE/amz-$userref->{'USERNAME'}-$docid.xml\n";
#
#		## keep an ATTEMPT counter so we don't get stuck on a document for too long.
#		if ($status eq '_FAILED_DUE_TO_FATAL_ERRORS_') {
#			$ERROR = "document $docid got fatal error response from amz.";
#			$pstmt = "update AMAZON_DOCS set RETRIEVED_GMT=1,RESPONSE_BODY=".$udbh->quote($ERROR).",ATTEMPTS=ATTEMPTS+1 where MID=$userref->{'MID'} and PRT=$userref->{'PRT'} and DOCID=".int($docid);
#			print STDERR $pstmt."\n";
#			$udbh->do($pstmt);
#			}
#		elsif ($status eq '_IN_PROGRESS_') {
#			warn "Document still in progress (attempts: $attempts created: ".&ZTOOLKIT::pretty_date($created,1).")";
#			}
#		elsif ($status eq '_SUBMITTED_') {
#			warn "Document submitted (attempts: $attempts created: ".&ZTOOLKIT::pretty_date($created,1).")";
#			}
#		elsif ($attempts>5) {
#			$ERROR = "Too many attempts on Document $docid";
#			$pstmt = "update AMAZON_DOCS set RETRIEVED_GMT=1,RESPONSE_BODY=".$udbh->quote($ERROR).",ATTEMPTS=ATTEMPTS+1 where MID=$userref->{'MID'} and PRT=$userref->{'PRT'} and DOCID=".int($docid);
#			print STDERR $pstmt."\n";
#			$udbh->do($pstmt);
#			warn ($ERROR);
#			}
#		elsif ($rdocid==0) {
#			warn "Document not available (yet) -- $status";
#			}
#		elsif ($status eq '_DONE_') { 
#			# the good case.
#			push @RDOCS, [ $doctype, $docid, $rdocid ];
#			}
#		elsif ($rdocid<0) {
...
## changed for MWS functionality
#sub getReadyDocument {
#	my ($userref,$doctype,$docid,$rdocid) = @_;
#
#	my ($udbh) = &DBINFO::db_user_connect($userref->{'USERNAME'});	
#
#	my ($ERROR,$xml) = &AMAZON3::getDocument($userref,$rdocid,$doctype);
#	#	#open F, ">/dev/shm/$rdocid.xml"; print F $xml; close F;
#	#	#die();
#	#	}
#	if ($ERROR) {
#		}
...
#	#	print "DONE\n";
#	#	print Dumper($msg); open F, ">/tmp/msgxml.dmp"; print F Dumper($msg); close F;
#		#	$VAR2 = {
#		#		 '.MessageID' => '1',
...
#
#	##
#	## go through each error report and flatten it, handle xml formatting stuff like 5xxx errors in this phase.
#	##		push flattened version onto @RESULTS
#	##
##	print "scalar: ".scalar(@reports)."\n";
##	print Dumper(\@reports);
#
#	foreach my $result (@reports) {
#		my $resultref = &ZTOOLKIT::XMLUTIL::SXMLflatten($result);
#		print "REPORT(s) docid=$docid rdocid=$rdocid doctype=$doctype msg:$resultref->{'.ResultMessageCode'}\n";
#		$resultref->{'docid'} = $docid;
#		$resultref->{'rdocid'} = $rdocid;
#		$resultref->{'doctype'} = $doctype;
##		print Dumper($resultref);
#
#		my ($src_file) = sprintf("%s/PRIVATE/amz-%s-%s.xml",ZOOVY::resolve_userpath($userref->{'USERNAME'}),$userref->{'USERNAME'},$docid);
#		$resultref->{'src_file'} = $src_file;
#
#		if ($docid==0) {
#			ZOOVY::confess($userref->{'USERNAME'},"You really think DOCID:$docid is valid?");
#			}
#
#		if ($resultref->{'.ResultMessageCode'} == '5000') {
#			## 5000 are generic XML processing errors, like badly formatted document sent. so we handle them higher.
#			## note: it's also worth mentioning that 5000 seem to be easily validatable/preventable errors.		
#			$ERROR = sprintf("FAIL|%s:%s",$resultref->{'.ResultMessageCode'},$resultref->{'.ResultDescription'});
#
#
#			if ($resultref->{'.ResultDescription'} =~ /^XML Parsing Error at Line ([\d]+), Column/) {
#				## alright, this is a really cheater way to parse through an amazon document. 
#				## it would make A LOT more sense to use XML writer to create[+save] a map of SKU/Docline#'s OR to simply
#				## CARP out errors and then fix 'em.. since I think all 5000 errors are correctable/preventable pre submission.
#				my ($USERNAME,$line) = ($userref->{'USERNAME'},$1);
#				my ($udbh) = &DBINFO::db_user_connect($USERNAME);
#
#				require ZOOVY;
#				my $MID = ZOOVY::resolve_mid($USERNAME);
#				my ($tmpf) = sprintf("/tmp/amz-%s-%s.xml",$USERNAME,$docid);
#				print "GOT 5000 ERROR - SEARCHING FILE: $src_file\n";
#				if (! -f $src_file) { 
#					Carp::cluck("WTF? No file: $src_file\n");
#					}
#
#				&ZOOVY::confess($userref->{'USERNAME'},"Amazon 5000 **PREVENTABLE RESPONSE***\nSRC FILE:$src_file\n".Dumper($resultref)."\n",justkidding=>1);
#
#				if (! -f $tmpf) {
#					system("/bin/cp $src_file $tmpf");
#					}
#
#				open(FILE, $tmpf);
#				my $content = '';
#				my $line_count = 0;
#				my $last_seen_sku = undef;
#				while (<FILE>) {
#					my $line = $_;
#					$line_count++;
#					next if ($line_count>$line); ## shortcut the rest of the file.
#
#					if ($line =~ /\<SKU\>(.*?)\<\/SKU\>/o) { $last_seen_sku = $1; }	# remember the last sku.
#					if ($line_count == $line) { $resultref->{'SKU'} = $last_seen_sku; }	
#					}
#				close(FILE);
#
#				if ($line_count==0) { 
#					Carp::cluck("Didn't get any lines in Amazon source document (this is probably a Zoovy issue)\n");
#					}
#	
#				&DBINFO::db_user_close();	
#				# ($resultref->{'SKU'}) = AMAZON3::find_XML_errors($userref->{'USERNAME'},$docid,$1);
#				if ($resultref->{'SKU'} ne '') { 
#					## could not ascertain nature of the error. .. should we warn here?
#					$ERROR = undef; 
#					}
#				}
#			}
#
#		if ($resultref->{'.ResultMessageCode'} eq '5002') {
#			## 5002 = please retry!
#			## requeue the document for dispatch.
#			$resultref->{'SKU'} = $resultref->{'.AdditionalInfo.SKU'};
#			if ($doctype eq '_POST_ORDER_ACKNOWLEDGEMENT_DATA_') {
#				## requeue for re-transmission, the queue mechanism will handle the errors and notifications.
#				my $pstmt = "update AMAZON_DOCS_QUEUE set DOCID=0,PROCESSED_GMT=0,LOCK_ID=0,LOCK_GMT=0,ATTEMPTS=ATTEMPTS+1 where MID=$userref->{'MID'} /* $userref->{'USERNAME'} */ and DOCID=$docid";
#				print STDERR $pstmt."\n";
#				$udbh->do($pstmt);
#				$resultref->{'_ignore'}++;		## we can ignore this error, it's just a guideline.
#				}
#			else {
#				$ERROR = "WARN|5002:failed $docid $doctype";
#				$UNKNOWN_ERRORS--;
#				}
#			#$VAR1 = {
#  			# '.MessageID' => 1,
#			# '.ResultMessageCode' => 5002,
#			# '.ResultDescription' => 'We are sorry, but we were unable to process this message. Please review the message and verify that it is accurate.  Then, please resubmit the message.',
#			# '.ResultCode' => 'Error'
#			#  };
#			$resultref->{'_ignore'}++;
#			}
#
#		if ($resultref->{'.ResultMessageCode'} eq '5006') {
#			# amazon has supsended this feed.
#			# This feed processing request was cancelled by an operator. Details: delayed processing  For further information please submit this full processing report to merchants-questions@amazon.com
#			$resultref->{'_ignore'}++;
#			}
#
#		if (($doctype eq '_POST_ORDER_FULFILLMENT_DATA_') && ($resultref->{'.ResultMessageCode'}==18030)) {
#			# '.ResultDescription' => 'Invalid OrderID or AmazonOrderID. For detailed steps on how to fix this error, see http://sellercentral.amazon.com/gp/help/25441',
#			$ERROR = "FAIL|What is an 18030 error?";
#			}
#		if ($resultref->{'.ResultCode'} eq 'Warning') {
#			## ignore warnings. they do not stop the product from syndicating and the messages can not be trusted
#			}
#		else {
...
#
#	##
#	## SANITY: at this point any errors are in @RESULTS
#	##				now we'll amend this with our own messaging about how to fix various issues.
#	##
#	foreach my $resultref (@RESULTS) {
#		next if ((defined $resultref->{'_ignore'}) && ($resultref->{'_ignore'}));
#
#		if (defined $resultref->{'.AdditionalInfo.SKU'}) {
#			## hurrah, this error identifies a SKU
#			# EXAMPLE: $VAR1 = {
#			# '.MessageID' => 1,
#			# '.ResultMessageCode' => 8065,
#			# '.AdditionalInfo.SKU' => 'A1C760181',
#			# '.ResultDescription' => 'Please provide a variation theme when creating a variation parent.',
#			# '.ResultCode' => 'Error'
#			#  };
#			$resultref->{'SKU'} = $resultref->{'.AdditionalInfo.SKU'};
#			delete $resultref->{'.AdditionalInfo.SKU'};	# no sense keep this duplicate.
#			}
#
#		#if ($resultref->{'.MessageID'}==0) {
#		#	## if an error doesn't have a specific msgid, then we won't know which SKU and we should fail the whole document.
#		#	}
#		#elsif	($resultref->{'.ResultMessageCode'} eq '13013') {
#		#	## This SKU does not exist in the Amazon.com catalog.  Your inventory data was not processed. For reasons why, and help fixing this, see http://sellercentral.amazon.com/gp/errorcode/13013
#		#	## eventually we should lookup the message code.
#		#	$resultref->{'FIXED'}++;
#		#	}
#		#elsif	($resultref->{'.ResultMessageCode'} eq '20005') {
#		#	## Cannot associate image with an existing product. For more details, see http://sellercentral.amazon.com/gp/errorcode/20005
#		#	$resultref->{'FIXED'}++;
#		#	}
#		#elsif	($resultref->{'.ResultMessageCode'} eq '18030') {
#		#	## Invalid OrderID or AmazonOrderID. For detailed steps on how to fix this error, see http://sellercentral.amazon.com/gp/help/25441
#		#	## /greatlookz:amz-2717522844.xml
#		#	$resultref->{'FIXED'}++;
#		#	}
#		#elsif	($resultref->{'.ResultMessageCode'} eq '18026') {
#		#	## No fulfillment data can be processed for this order, as it was declined, cancelled, or not yet processed.
#		#	## this probably means the custoemr cancelled the order on amazon's side but failed to let us know.
#		#	$resultref->{'FIXED'}++;
#		#	}
#		#elsif	($resultref->{'.ResultMessageCode'} eq '18021') {
#		#	## Unacceptable quantity specified for this order. For reasons why, and help fixing this, see 
#		#	## http://sellercentral.amazon.com/gp/help/25341
#		#	$resultref->{'FIXED'}++;
#		#	}
#		#elsif ($resultref->{'.ResultMessageCode'} eq '8065') {
#		#	# Please provide a variation theme when creating a variation parent.
#		#	$resultref->{'FIXED'}++;
#		#	}				
#		#elsif ($resultref->{'.ResultMessageCode'} eq '8026') {
#		#	# Seller is not authorized to list products in this category. For more details, see http://sellercentral.amazon.com/gp/errorcode/8026
#		#	$resultref->{'FIXED'}++;
#		#	}				
#		#elsif ($resultref->{'.ResultMessageCode'} eq '8042') {
#		#	## SKU \'TORANI_SYR_AMARETTO\' could correspond to multiple ASINs in the Amazon catalog. Please review the possible matches below.	If your product is the same as the product on one of these detail pages, please modify your product data to reflect the Amazon catalog values and resubmit. If your product is different than what is on these detail pages, please provide more data and resubmit.	ASIN B000GZCVWO  \'brand_name\' Merchant value: \'R. Torre & Co.\' Amazon catalog value: \'Torani\'  \'manufacturer\' Merchant value: \'R. Torre & Co.\' Amazon catalog value: \'Torani\'  \'item_name\' Merchant value: \'Torani Amaretto Syrup - Italian Syrup 750ml - 25.4oz\' Amazon catalog value: \'Amaretto Syrup by Torani\'.	ASIN B001E0FO9C  \'brand_name\' Merchant value: \'R. Torre & Co.\' Amazon catalog value: \'(TORANI) R TORRE AND CO\'  \'manufacturer\' Merchant value: \'R. Torre & Co.\' Amazon catalog value: \'TORANI\'  \'item_name\' Merchant value: \'Torani Amaretto Syrup - Italian Syrup 750ml - 25.4oz\' Amazon catalog value: \'TORANI AMARETTO 750 ML\'.	For details, see http://sellercentral.amazon.com/gp/errorcode/8042
#		#	$resultref->{'FIXED'}++;
#		#	}				
#		#elsif ($resultref->{'.ResultMessageCode'} eq '8041') {
#		#	## SKU \'OCRC_03\' appears to correspond to ASIN B000Q6OCFC, but some of the information submitted does not match the product information that is already in the Amazon catalog for that ASIN.	If your product is the same as the one on this detail page, please modify your product data to reflect the following Amazon catalog values and resubmit. If your product is different than what is on the detail page, please provide more data and resubmit.	\'item_name\' Merchant value: \'Ethiopia Idido Misty Valley ~ 12 oz.\' Amazon catalog value: \'Brazil Sul De Minas\'.	For details, see http://sellercentral.amazon.com/gp/errorcode/8041
#		#	$resultref->{'FIXED'}++;
#		#	}				
#		#elsif ($resultref->{'.ResultMessageCode'} eq '8008') {
#		#	## We cannot recognize your child SKU: [TORANI_SYR_AMARETTO]. For help on how to proceed, see http://sellercentral.amazon.com/gp/errorcode/8008
#		#	$resultref->{'FIXED'}++;
#		#	}
#		#elsif ($resultref->{'.ResultMessageCode'} eq '8007') {
#		#	## We cannot recognize your parent SKU: [1245]. For more information, see http://sellercentral.amazon.com/gp/errorcode/8007.
#		#	$resultref->{'FIXED'}++;
#		#	}
#		#elsif ($resultref->{'.ResultMessageCode'} eq '8004') {
#		#	## The Product with SKU [OCRC_03] may be identical to existing ASINs ([B000Q6OCFC, ]) based on these attributes: [item_name, ]. For details, see http://sellercentral.amazon.com/gp/errorcode/8004
#		#	$resultref->{'FIXED'}++;
#		#	}
#
#		if (($resultref->{'SKU'} eq '') && ($resultref->{'.MessageID'}>0) && ($doctype eq '_POST_INVENTORY_AVAILABILITY_DATA_')) {
#			## inventory messages have the SKU MsgID set
#			my ($sTB) = &ZOOVY::resolve_lookup_tb($userref->{'USERNAME'});
#			# my $pstmt = "select SKU from AMAZON_PID_UPCS where MID=$userref->{'MID'} /* $userref->{'USERNAME'} */ and LASTDOC_INVENTORY=".int($docid)." and LASTDOC_INVENTORY_MSGID=".int($resultref->{'.MessageID'});
#			my $pstmt = "select PID,INVOPTS from $sTB where MID=$userref->{'MID'} /* $userref->{'USERNAME'} */ and LASTDOC_INVENTORY=".int($docid)." and LASTDOC_INVENTORY_MSGID=".int($resultref->{'.MessageID'});
#			print $pstmt."\n";
#			my ($PID,$INVOPTS) = $udbh->selectrow_array($pstmt);
#			$resultref->{'SKU'} = sprintf("%s%s",$PID,($INVOPTS?":$INVOPTS":""));
#			}
#
#		if (($resultref->{'SKU'} eq '') && ($resultref->{'.ResultDescription'} =~ /SKU \[(.*?)\]/)) {
#			## a lot of product error responses don't contain the SKU, but do contain it in the description
#			## ex: The Product with SKU [AERO_FILTER] may be identical to existing ASINs ([B0018S2Y0M, B001RQM11M, ]) based on these attributes: [brand_name, manufacturer, matching_part_number, item_name, ]. For details, see http://sellercentral.amazon.com/gp/errorcode/8004
#			$resultref->{'SKU'} = $1;
#			}
#
#		if (($resultref->{'SKU'} eq '') && ($resultref->{'.ResultDescription'} =~ /SKU \'(.*?)\'/)) {
#			## apparently they don't like common naming conventions .
#			## SKU \'AERO_FILTER\' could correspond to multiple ASINs in the Amazon catalog. Please review the possible matches below.	If your product is the same as the product on one of these detail pages, please modify your product data to reflect the Amazon catalog values and resubmit. If your product is different than what is on these detail pages, please provide more data and resubmit.	ASIN B0018S2Y0M  \'gtin\' Merchant value: \'00085276000817\' Amazon catalog value: [].	ASIN B001RQM11M  \'brand_name\' Merchant value: \'Aerobie\' Amazon catalog value: \'Julien Beaudoin\'  \'manufacturer\' Merchant value: \'Aerobie\' Amazon catalog value: \'Julien Beaudoin\'  \'matching_part_number\' Merchant value: \'AEROPRESS\' Amazon catalog value: \'\'  \'item_name\' Merchant value: \'AeroPress Coffee & Espresso Maker Micro Filters - 350 Count\' Amazon catalog value: \'Seven Legs on Casters - Queen Size Bolt-On Bed Frame By Julien Beaudoin\'.	For details, see http://sellercentral.amazon.com/gp/errorcode/8042
#			$resultref->{'SKU'} = $1;
#			}
#
#
#		if ($resultref->{'SKU'} ne '') {
#			## this error was able to mapped to a particular SKU. -- we'll use ERROR later.
#			$resultref->{'ERROR'} = sprintf("%s:%s",$resultref->{'.ResultMessageCode'},$resultref->{'.ResultDescription'});
#			$UNKNOWN_ERRORS--;
#			}
#		elsif ($doctype eq '_POST_ORDER_FULFILLMENT_DATA_') {
#			## these document types will never have a sku
#			}
#		else {
#			## normal message, not sure what todo here.
#			my $justkidding = 0;
#
#			## XML Parsing Fatal Error at Line 9054, Column 3: The element type "AmazonEnvelope" must be terminated by the matching end-tag "</AmazonEnvelope>".
#			## okay fuck this.. lets do this the hard way
#			my ($udbh) = &DBINFO::db_user_connect($userref->{'USERNAME'});
#			my $pstmt = "update AMAZON_DOCS set ATTEMPTS=ATTEMPTS+1 where DOCID=".int($docid)." and MID=".int($userref->{'MID'});
#			$udbh->do($pstmt);
#
#			$pstmt = "select ATTEMPTS from AMAZON_DOCS where DOCID=".int($docid)." and MID=".int($userref->{'MID'});
#			my ($attempts) = $udbh->selectrow_array($pstmt);
#			if ($attempts>5) { 
#				## after 5 attempts, we're done with this bizatch.
#				$justkidding++;
#				}
#			&DBINFO::db_user_close();
#
#			#if (($resultref->{'.ResultMessageCode'} == 13013) && ($userref->{'USERNAME'} eq 'espressoparts2')) { 
#			#	$justkidding++; 
#			#	}
#			#if (($resultref->{'.ResultMessageCode'} == 8065) && ($userref->{'USERNAME'} eq 'espressoparts2')) { 
#			#	$justkidding++; 
#			#	}
#			
#		if ($resultref->{'.ResultMessageCode'} == 18030) {
#			## error 18030 - if an order is cancelled on amazon, then they won't acknowledge it.
#			$justkidding++;
#			}
#		elsif (($resultref->{'.ResultMessageCode'} == 5000) && ($resultref->{'.ResultDescription'} eq 'The value provided for SKU is empty or contains only whitespace.')) {
#			$justkidding++;
#			}
#		elsif ($resultref->{'.ResultMessageCode'} == 18025) {
#			$justkidding++;
#			## Order id 002-3298665-2059409 cannot be acknowledged, because it is either still pending or has been cancelled
#			}
#		elsif ($resultref->{'.ResultCode'} eq 'Warning') {
#			## we can safely ignore all warnings.
#			$justkidding++;
#			}
#		else {
#			&ZOOVY::confess($userref->{'USERNAME'},
#				(($justkidding)?"ENOUGH ALREADY! ($attempts) ":"ATTEMPT $attempts ").
...
#				}
#
#			if ($justkidding) { $UNKNOWN_ERRORS--; $resultref->{'_ignore'}++; }
#			}
#		
#		if ($resultref->{'SKU'} eq '') {
#			## set the global error for the document since we don't have a SKU to associate this to.
#			$ERROR = $resultref->{'ERROR'};	
#			}
#	
#		# print '!!! RESULT: '.Dumper($resultref)."\n";
#		}
#
#	## results is a hashref, it has .MessageID, .ResultMessageCode, .AdditionalInfo.SKU, .ResultDescription
#
#	if (@RESULTS>0) {
...
#
#	if ($UNKNOWN_ERRORS>0) {
#		if ($ERROR ne '') { $ERROR = "ZA1000:Had $UNKNOWN_ERRORS unknown errors"; }
#		ZOOVY::confess($userref->{'USERNAME'},"AMAZON Got an unsuccessful document: ".Dumper({'errs-remain'=>$UNKNOWN_ERRORS}));
#		die();
#		}
#	
#	#foreach my $result (@RESULTS) {
#	#	&AMAZON3::zlog($userref,ERROR=>$ERROR,DOCTYPE=>$doctype,DOCID=>$docid,RDOCID=>$rdocid,resultref=>$result);
#	#	}
#
#	if ($rdocid < 0) {
#		## internal error, probably a bad response document. no need to go to confession.
#		}
#	elsif ($rdocid==0) {
#		## document wasn't ready
#		}
#	elsif ($ERROR eq '') {
#		## we intentionally don't set RETREIVED_GMT here, that way if an error happens later we'll still retry.
#		my $pstmt = "update AMAZON_DOCS set RESPONSE_DOCID=$rdocid,RESPONSE_BODY=".$udbh->quote($xml)." where MID=$userref->{'MID'} /* $userref->{'USERNAME'} */ and DOCID=$docid";
#		print $pstmt."\n";
#		$udbh->do($pstmt);
#		}
#	else {
#		ZOOVY::confess($userref->{'USERNAME'},$ERROR,justkidding=>1);
#		}
#
#	&DBINFO::db_user_close();
#
#	print "getReadyDocument FINAL ERROR:$ERROR\n";
