...
sub digest {
	...
	## print STDERR "DIGEST: ".$self->cartid()." $digest ".join("|",@DIGESTABLE)."\n";

	

#	my %SETS = (
#		'@SHIPPING'=>[ 'want/payby', 'sum/spc_total', 'want/ship_selector', 'sum/shp_total', 'want/bnd_purchased', 'want/bnd_total', 'cart/buysafe_val', 'ship/postal', 'ship/address1', 'ship/city',' ship_state', 'bill/countrycode', '*stuff' ],
#		);

#	$c .= (($payby eq 'COD') || ($payby eq 'CHKOD'))?1:0;
#	## position 1 (dec 2) = payment method changed (or rather specifically the payment fee changed)
#	$c .= '|'.$self->fetch_property('ship.spc_total');
#	## position 2 (dec 4) = shipping changed.
#	my $shipmethod = $self->fetch_property('ship.selected_id');
#	$shipmethod =~ s/[\W\s]+//gs;	# removes funny punctuation, etc.
#	$c .= '|'.$shipmethod.$self->fetch_property('ship.selected_price');	
#	## position 3 (dec 8) = insurance
#	$c .= '|'.int($self->fetch_property('ship.ins_total')).'-'.int($self->fetch_property('ship.ins_purchased'));	
#	## position 4 (dec 16) = bonding 
#	$c .= '|'.int($self->fetch_property('ship.bnd_purchased')).'/'.$self->fetch_property('ship.bnd_total').'/'.($self->fetch_property('ship.buysafe_val'));	
#	## position 5 (dec 32) = shipping destination
#   my $zip = uc($self->{'data.ship_zip'});
#   $zip =~ s/[^A-Z0-9]+//gs;
#   $zip = substr($zip,0,5);
#	## note: ship_address1 must be there for PO boxes (if address changes, so must shipping rules)
#	$c .= '|'.$self->{'data.ship_address1'}.$self->{'data.ship_country'}.$self->{'data.ship_state'}.$zip;
#	## position 6 (dec 64) = billing location
#	$c .= '|'.$self->{'data.bill_country'};
#	## position 7 (dec 128) = items in cart
#	my ($s) = $self->{'stuff'};
#	$c .= '|';
#	if (ref($s) eq 'STUFF') {
#		$c .= $s->digest();
#		## DID NOT TAKE INTO ACCOUNT QUANTITY
#		# $c .= join(',',sort $s->stids());
#		}
...
##
sub verify_payment {
	my ($self, $paymentref, $webdbref) = @_;

	my @ISSUES = ();
	my $payby = $self->in_get('will/payby');	
	if ($payby eq '') {
		push @ISSUES, [ 'ERROR', 'no_payment_method', 'chkout.payby', 'cart variable chkout.payby is not set.' ];
		}

	my ($methodref) = &ZPAY::lookup_method($payby);
	my ($method,$pretty) = ();
	if (defined $methodref) {
		# $methodref = [ 'CREDIT', 'Credit Card ' ];
		($method,$pretty) = @{$methodref};
		}

	# print STDERR "PAYMENT_REF: ".Dumper($methodref,$method);

	if ((not defined $method) || ($method eq '')) {
		push @ISSUES, [ 'ERROR', 'no_payment_method', 'chkout.payby', 'payment method is required' ];
		}
	elsif ($method eq 'CREDIT') {
		#check credit card number isn't blank
		...
		elsif ($paymentref->{'CC'} =~ /[^\d]+/) {
			push @ISSUES, [ 'ERROR', 'cc_invalid_digits', 'payment.cc', 'Credit card number contains space or other non-numeric characters.' ]
			}
		elsif (not &ZPAY::cc_verify_length($paymentref->{'CC'})) {
			push @ISSUES, [ 'ERROR', 'cc_invalid_length', 'payment.cc', 'Credit card number does not have the appropriate length.' ];
			}
		elsif (not &ZPAY::cc_verify_checksum($paymentref->{'CC'})) {
			push @ISSUES, [ 'ERROR', 'cc_invalid_checksum', 'payment.cc', 'Credit card number supplied does not have a valid checksum (please verify the digits).' ];
			}
		elsif (not &ZPAY::cc_verify_type_for_merchant($self->username(),$paymentref->{'CC'},$webdbref)) {
			push @ISSUES, [ 'ERROR', 'cc_invalid_type', 'payment.cc', 'Credit card is not a type this merchant accepts.' ];
			}
			
		if (not &ZPAY::cc_verify_expiration($paymentref->{'MM'},$paymentref->{'YY'})) {
			push @ISSUES, [ 'ERROR', 'cc_expired', 'payment.yy', 'Credit card has expired.' ];
			}
		...
		#	}
		if (defined($webdbref->{'cc_cvvcid'}) && ($webdbref->{'cc_cvvcid'} > 0)) {
			if ($paymentref->{'CV'}) { 
				# CIDCVV is requested
				if (not &ZPAY::cc_verify_cvvcid($paymentref->{'CC'},$paymentref->{'CV'})) {
					push @ISSUES, [ 'ERROR', 'cc_cvv_invalid', 'payment.cv', 'CID or CVV number is invalid for card type.' ];
					}
				}
			elsif ($webdbref->{'cc_cvvcid'} == 2) {
				# CIDCVV is required
				push @ISSUES, [ 'ERROR', 'cc_cvv_required', 'payment.cv', 'CID or CVV number must be provided.' ];
				}
			}

		}
	elsif ($method eq 'PO') {
		if (&ZTOOLKIT::wordlength($self->in_get('want/po_number')) < 1) {
			push @ISSUES, [ 'ERROR', 'po_required', 'chkout.po_number', 'You must provide a Purchase Order number' ];
			}
		}
	elsif ($method eq 'ECHECK') {
		if (&ZTOOLKIT::wordlength($paymentref->{'EB'}) < 4) {
			push @ISSUES, [ 'ERROR', 'ec_eb_required', 'payment.echeck_bank_name', 'You must provide the name of the bank which of the checking account' ];
			}
...
##
#sub verify_checkout {
#	my ($self,$STAGE,$SREF) = @_;
#
#	if (ref($SREF) ne 'SITE') { warn Carp::cluck("CART2::verify_checkout requires a SITE object be passed"); }
#
#	my $webdbref = $SREF->webdbref();
#
#	## STAGES are:
#	my @fields = ();
#	## @fields format is:
#	## [0] group/checkout stage
#	## [1] form field/property 'most responsible' or blank for unknown
#	## [2] is_required
#	## [3] hash of additional properties
#
#	## STAGE can be any of the following:
#	
#	my @ISSUES = ();
#	## @ISSUES is the following format:
#	## [0] 	ERROR|WARNING|ISE?
#	##	[1]	message source/cause
#	##	[2]	property 'most responsible' or blank for unknown
#	##	[3]	error message (or blank for not set)
#	##	[4]	hashref of properties of the error, may contain msgid for a reference to SITE::msgs
#
#
#	my %VALIDATION_GROUPS = ();
#
#	if (($STAGE eq 'CHOOSE') || ($STAGE eq 'CHOOSE.NEW')) {
#		## no validation here
#		}
#	elsif ($STAGE eq 'CHOOSE.LOGIN') {
#		## no validation here either
#		}
#	elsif ($STAGE eq 'BILLING_LOCATION') {
#		$VALIDATION_GROUPS{'BILLING'}++;
#		}
#	elsif ($STAGE eq 'SHIPPING_LOCATION') {
#		$VALIDATION_GROUPS{'SHIPPING'}++;
#		}
#	elsif ($STAGE eq 'ORDER_CONFIRMATION') {
#		$VALIDATION_GROUPS{'BILLING'}++;
#		$VALIDATION_GROUPS{'SHIPPING'}++;
#		$VALIDATION_GROUPS{'CONFIRM'}++;
#		if ($self->in_get('want/create_customer')>0) {
#			$VALIDATION_GROUPS{'NEW_CUSTOMER'}++;			
#			}
#		}
#	elsif ($STAGE eq 'ADMIN') {
#		## NO VALIDATION GROUPS FOR ADMIN
#		}
#	elsif ($STAGE eq 'PAYMENT_INFORMATION') {
#		$VALIDATION_GROUPS{'BILLING'}++;
#		$VALIDATION_GROUPS{'SHIPPING'}++;
#		$VALIDATION_GROUPS{'PAYMENT'}++;
#		$VALIDATION_GROUPS{'CONFIRM'}++;
#		}
#	elsif ($STAGE eq 'PAYPALEC') {
#		$VALIDATION_GROUPS{'SHIPPING'}++;
#		$VALIDATION_GROUPS{'PAYMENT'}++;
#		}
#	#elsif ($STAGE eq 'CALLCENTER') {
#	#	## verify everything
#	#	$VALIDATION_GROUPS{'BILLING'}++;
#	#	$VALIDATION_GROUPS{'SHIPPING'};
#	#	$VALIDATION_GROUPS{'PAYMENT'};
#	#	}
#	else {
#		print STDERR "UNKNOWN/UNHANDLED VERIFY_CHECKOUT_STAGE: $STAGE\n";
#		}
#
#	my $phone_required = 0;
#	if ($webdbref->{'chkout_phone'} eq 'UNREQUESTED') { $phone_required = -1; }
#	elsif ($webdbref->{'chkout_phone'} eq 'REQUIRED') { $phone_required = 1; }
#	elsif ($webdbref->{'chkout_phone'} eq 'OPTIONAL') { $phone_required = 0; }
#
#	my $payby = $self->in_get('want/payby');
#	if ($payby eq 'PAYPALEC') { $phone_required = 0; }
#
#	##
#	## NOTE: authentication validation should be done in checkout.
#	##
#
#	push @fields, [ 'BILLING', 'bill/firstname', 1, { 'type'=>'word', 'min_length'=>1, 'max_length'=>20, 'msgid'=>'chkout_error_invalid_firstname' } ];
#	push @fields, [ 'BILLING', 'bill/middlename', 0, { 'type'=>'word', 'min_length'=>0, 'max_length'=>10, } ];
#	push @fields, [ 'BILLING', 'bill/lastname', 1, { 'type'=>'word', 'max_length'=>20, 'msgid'=>'chkout_error_invalid_lastname' } ];
#	push @fields, [ 'BILLING', 'bill/email', 1, { 'max_length'=>65, 'type'=>'email', 'msgid'=>'chkout_error_email_invalid' } ];
#	if ($payby eq 'PAYPALEC') {
#		}
#	else {
#		push @fields, [ 'BILLING', 'bill/address1', 1, { 'type'=>'address', 'msgid'=>'chkout_error_invalid_address1' } ];
#		push @fields, [ 'BILLING', 'bill/address2', 0, { 'type'=>'word' } ];
#		push @fields, [ 'BILLING', 'bill/countrycode', 0, { 'type'=>'country' } ];
#		push @fields, [ 'BILLING', 'bill/city', 1, { 'type'=>'word', 'min_length'=>2, 'msgid'=>'chkout_error_invalid_bill_city' } ];
#		## bill_state, bill_zip are conditionally required based on ship_country
#		push @fields, [ 'BILLING', 'bill/region', 0, { 'type'=>'state', 'country'=>'bill/countrycode', 'msgid'=>'chkout_error_invalid_bill_state' } ];
#		push @fields, [ 'BILLING', 'bill/postal', 0, { 'type'=>'zip', 'country'=>'bill/countrycode', 'msgid'=>'chkout_error_invalid_bill_zip' } ];
#		}
#
#	if ($phone_required >= 0) {
#		push @fields, [ 'BILLING', 'bill/phone', $phone_required, { 'type'=>'phone', 'msgid'=>'chkout_error_phone_required'} ];
#		}
#
#	if ($payby eq 'PAYPALEC') {
#		## paypalec has no address validation, it's special sauce.
#		}
#	elsif ($self->in_get('will/bill_to_ship')) {
#		## if we're doing billing to check, we need to copy billing to shipping address then validate shipping address1
#		push @fields, [ 'BILLING', 'will/bill_to_ship', 1, { 'type'=>'chkout-billtoship' } ];
#		push @fields, [ 'BILLING', 'ship/address1', 1, { 'type'=>'address', 'msgid'=>'chkout_error_invalid_address1' } ];
#		}
#	elsif (not $self->in_get('want/bill_to_ship')) {
#		## don't validate shipping fields if we're doing bill_to_ship (since they didn't/can't specify shipping fields)
#		push @fields, [ 'SHIPPING', 'ship/firstname', 1, { 'type'=>'word', 'max_length'=>20, 'msgid'=>'chkout_error_invalid_firstname' } ];
#		push @fields, [ 'SHIPPING', 'ship/middlename', 0, { 'type'=>'word', 'max_length'=>1 } ];
#		push @fields, [ 'SHIPPING', 'ship/lastname', 1, { 'type'=>'word', 'max_length'=>20, 'msgid'=>'chkout_error_invalid_lastname' } ];
#		push @fields, [ 'SHIPPING', 'ship/address1', 1, { 'type'=>'address', 'msgid'=>'chkout_error_invalid_address1' } ];
#		push @fields, [ 'SHIPPING', 'ship/address2', 0, { 'type'=>'word' } ];
#		push @fields, [ 'SHIPPING', 'ship/countrycode', 0, { type=>'country' } ];
#		push @fields, [ 'SHIPPING', 'ship/city', 1, { 'type'=>'word', 'min_length'=>2, 'msgid'=>'chkout_error_invalid_ship_city' } ];
#		## ship_state, ship_zip are conditionally required based on ship_country, so they are not 'required' as in non-blank
#		push @fields, [ 'SHIPPING', 'ship/region', 0, { 'type'=>'state', 'country'=>'ship/countrycode', 'msgid'=>'chkout_error_invalid_ship_state' } ];
#		push @fields, [ 'SHIPPING', 'ship/postal', 0, { 'type'=>'zip', 'country'=>'ship/countrycode', 'msgid'=>'chkout_error_invalid_ship_zip' } ];
#		if ($phone_required>=0) {
#			push @fields, [ 'SHIPPING', 'ship/phone', $phone_required, { 'type'=>'phone', 'msgid'=>'chkout_error_phone_required'} ];	
#			}
#		}
#
...
#		}
#	elsif ($self->customerid()>0) {
#		## we're already logged in, no need to check for password.
#		}
#	elsif ($self->in_get('want/create_customer')==0) {
#		}
#	else {
#		push @fields, [ 'NEW_CUSTOMER', 'want/new_password', 1, { type=>'password', msgid=>'chkout_error_invalid_password'} ];
#		push @fields, [ 'NEW_CUSTOMER', 'want/new_password2', 1, { type=>'password', match=>'want/new_password', msgid=>'chkout_error_invalid_password2' } ];
#		push @fields, [ 'NEW_CUSTOMER', 'want/recovery_hint', 0, { type=>'number', 'min'=>1, 'max'=>10, } ];
#		push @fields, [ 'NEW_CUSTOMER', 'want/recovery_answer', 1, { type=>'word', 'min_length'=>1, 'max_length'=>20, msgid=>'chkout_error_invalid_recovery_answer' } ];
#		}
#
#	## NOTE: need to update chk
#	push @fields, [ 'CONFIRM', 'want/shipping_id', 1, { type=>'shipping' } ];
#	push @fields, [ 'CONFIRM', 'want/order_notes', 0, { type=>'word' } ];
#	push @fields, [ 'CONFIRM', 'want/po_number', 0, { type=>'word' } ];
#	push @fields, [ 'CONFIRM', 'want/erefid', 0, { type=>'word' } ];
#
#	## these are two optional settings that can be selected at the confirm stage
#	#push @fields, [ 'CONFIRM', 'ship.bnd/purchased', 0, { type=>'boolean', optional=>1 } ];
#	#push @fields, [ 'CONFIRM', 'ship.ins_purchased', 0, { type=>'boolean', optional=>1 } ];
#
#	push @fields, [ 'PAYMENT', 'will/payby', 1, { type=>'payment' } ];
#
##	print STDERR Dumper($self);
##	die();
#
#	foreach my $try (@fields) {
#		my ($group,$field,$is_required,$properties) = @{$try};
#
#		next if (not defined $VALIDATION_GROUPS{$group});	# skip groups that we aren't currently looking for.		
#
#		my $val = $self->in_get($field);
#		if (($is_required) && ($val eq '')) {
#			push @ISSUES, [ 'ERROR', 'is_required', $field, '', $properties ];
#			}
#		elsif ($properties->{'type'} eq 'number') {
#			}
#		elsif ($properties->{'type'} eq 'word') {
#			if ((defined $properties->{'min_length'}) && (&ZTOOLKIT::wordlength($val) < $properties->{'min_length'})) {
#				push @ISSUES, [ 'ERROR', 'min_length', $field, '', $properties ];
#				}
#			elsif ((defined $properties->{'max_length'}) && (&ZTOOLKIT::wordlength($val) > $properties->{'max_length'})) {
#				push @ISSUES, [ 'ERROR', 'min_length', $field, '', $properties ];
#				}
#			}
#		elsif ($properties->{'type'} eq 'email') {
#			if (not &ZTOOLKIT::validate_email($val)) {
#				push @ISSUES, [ (($is_required)?'ERROR':'WARNING'), 'invalid_email', $field, '', $properties ];
#				}
#			}
#		elsif ($properties->{'type'} eq 'address') {
#			## eventually we might want to check address structure here.
#			my $check_for_pobox = 0;
#
#			if ((not defined $webdbref->{'chkout_deny_ship_po'}) || ($webdbref->{'chkout_deny_ship_po'}==0)) {
#				## po boxes are no problem
#				}
#			elsif ($field eq 'ship/address1') {
#				## poboxes are a problem, and we're validating a shipping address
#				$check_for_pobox++;
#				}
#			elsif (($field eq 'bill/address1') && ($self->in_get('will/bill_to_ship')) ) {
#				## since shipping and billing are the same, we need to validate for poboxes.
#				$check_for_pobox++;
#				}
#
#			if (($check_for_pobox) && ($self->is_pobox()>0)) {
#				# only check pobox address on shipping (not billing)
#				push @ISSUES, [ 'ERROR', 'pobox_not_allowed', $field, '', { 'msgid'=>'chkout_error_pobox_not_allowed' } ];
#				}
#
#			}
#		elsif ($properties->{'type'} eq 'phone') {
#			## NOTE: phone number format changes based on country
#			#if (not &ZTOOLKIT::validate_phone($self->in_get('bill/phone'),$self->in_get('data.bill_country'))) { 
#			#	$errors{'bill_phone'} = $SITE::msgs->get('chkout_error_phone_required');
#			#	}
#			if ((not $is_required) && ($val eq '')) {
#				## it's not required, and it's blank
#				}
#			elsif (not &ZTOOLKIT::validate_phone($val)) {
#				push @ISSUES, [ (($is_required)?'ERROR':'WARNING'), 'invalid_phone', $field, '', $properties ];
#				}
#			}
#		elsif ($properties->{'type'} eq 'password') {
#			if (&ZTOOLKIT::wordlength($val) < 4) {
#				push @ISSUES, [ 'ERROR', 'password_to_short', $field, 'Password must be at least 4 characters long.' ];
#				}
#			if ((defined $properties->{'match'}) && ($val ne $self->in_get('want/new_password')) ) {
#				push @ISSUES, [ 'ERROR', 'password_does_not_match', $field, 'Passwords must match' ];
#				}
#			}
#		elsif ($properties->{'type'} eq 'zip') {
#			my $country = $self->in_get($properties->{'country'});
#			if (not defined $val) { $val = ''; }
#			
#			if (&ZSHIP::check_zip($val,$country)) {
#				## yay, good zip code.
#				}
#			elsif ($country eq '') {
#				push @ISSUES, [ 'ERROR', 'invalid_us_zip', $field, 'You must provide a valid ZIP or ZIP+4 code.' ];
#				}
#			else {
#				push @ISSUES, [ 'ERROR', 'invalid_us_zip', $field, sprintf("You must provide a valid postal code for country:%s.",$country) ];
#				}
#			}
#		elsif ($properties->{'type'} eq 'country') {
#			# my %countries = &ZSHIP::fetch_int_destinations_by_merchant($self,$webdbref);
#			my $iso = undef;
#			foreach my $dst (@{&ZSHIP::available_destinations($self,$webdbref)}) {
#				if (uc($val) eq uc($dst->{'Z'})) { $iso = $dst->{'ISO'}; }
#				if (uc($val) eq uc($dst->{'ISO'})) { $iso = $val; }
#				}
#			
#			if ($val eq '') {
#				## cheap hack to eliminate cases where buyer managed to get a country in and we only support us destinations
#				## us is always allowed.
#				$iso = 'US'; 
#				}
#			
#			if (not defined $iso) {
#				push @ISSUES, [ 'ERROR', 'unsupported_country', $field, sprintf('Country (%s) is not supported.',$val) ];
#				}
#			}
#		elsif ($properties->{'type'} eq 'chkout-billtoship') {
#			## chkout.billtoship: sort of a unique field, it's required in the sense that we need a 1 or 0 to be passed.
#			if ($val eq 'on') { $val = 1; }
#
#			my ($payby) = $self->in_get('will/payby');	
#			if ($payby eq 'PAYPALEC') {
#				## paypalec has special address requirements.
#				}
#			elsif ($val eq '') {
#				push @ISSUES, [ 'ERROR', 'billtoship_required', $field, 'chkout.billtoship is a required field and must be passed', {} ];
#				}
#			elsif (int($val) == 0) {
#				}
#			elsif (int($val) == 1) {
#				## copy billing over shipping address. for chkout.bill_to_ship
#				foreach my $x ('firstname','middlename','lastname','company','email','address1','address2','countrycode','city','region','postal','phone') {
#					$self->in_set("ship/$x",$self->in_get("bill/$x"));
#					}
# 				}
#
#			}
#		elsif ($properties->{'type'} eq 'payment') {
#			my ($method,$pretty) = &ZPAY::lookup_method($val);
#			if ($method eq '') {
#				push @ISSUES, [ 'ERROR', 'invalid_payment', $field, 'Invalid payment type', {} ];
#				}
#			## NOTE: the line below has never been tested!
#			#if ( ($method eq 'PAYPALEC') && ($self->in_get('chkout.paypalec') eq '') ) {
#			#	push @ISSUES, [ 'ERROR', 'invalid_paypalec', $field, 'Paypal EC internal error', {} ];
#			#	}
#			}
#		elsif ($properties->{'type'} eq 'state') {
#			my $country = $self->in_get($properties->{'country'});
#			if (&ZSHIP::check_state($val,$country)) {
#				}
#			elsif ($payby eq 'PAYPALEC') {
#				## we got this state from paypal, so we should trust it. (sometimes they return no state
#				}
#			elsif ($country eq '') {
#				push @ISSUES, [ 'ERROR', 'need_us_state', $field, 'You must enter a valid US state.', {} ];
#				}
#			elsif ($country eq 'Canada') {
#				push @ISSUES, [ 'ERROR', 'need_ca_state', $field, 'You must enter a valid Canadian province.', {} ];
#				}
#			elsif ($country eq 'United Kingdom') {
#				push @ISSUES, [ 'ERROR', 'need_uk_state', $field, 'You must enter a province (for UK orders, use your county here)', {} ];
#				}
#			else {
#				push @ISSUES, [ 'ERROR', 'need_uk_state', $field, 'You must enter a state/province.', {} ];
#				}
#			}
#		elsif ($properties->{'type'} eq 'shipping') {
#			# $self->shipping();
#			my $shipmethods = $self->shipmethods();
#			# print STDERR Dumper($self);
#
#			if ((not defined $shipmethods) || (ref($shipmethods) ne 'ARRAY')) {
#				push @ISSUES, [ 'ISE', 'shipping_ise', $field, 'Internal error to shipping application.', {} ];				
#				}
#			elsif ($val eq '') {
#				push @ISSUES, [ 'ERROR', 'shipping_not_selected', $field, 'You must select a shipping method.', {} ];
#				}
#			elsif (scalar(@{$shipmethods})==0) {
#				push @ISSUES, [ 'ISE', 'shipping_not_configured', $field, 'No shipping methods are available based on the store configuration.', {} ];
#				}
#			else {
#				## SANITY: they have a shipping method specified, and shipping methods are available, lets check for a match
#				my $found = 0;
#				# print STDERR "VAL:$val\n";
#				foreach my $shipmethod (@{$shipmethods}) {
#					# print STDERR Dumper($shipmethod)."\n";
#					if ($val eq $shipmethod->{'id'}) { $found++; }
#					if ($val eq $shipmethod->{'pretty'}) { $found++; }
#					if ($val eq $shipmethod->{'name'}) { $found++; }
#					}
#				if (not $found) {
#					push @ISSUES, [ 'ERROR', 'shipping_not_available', $field, "The shipping method specified is not available ($val)", {} ];
#					}
#				}
#			}
#		else {
#			push @ISSUES, [ 'WARNING', 'unknown_type', $field, "field:$field did not have a type", {} ];
#			}
#		}
#
#
...
#		# my ($type, $src, $field, $msg, $properties) = @{$issue};
#
#		if ($issue->[3] ne '') {
#			## it's already got a message set!
#			}
#		elsif ($issue->[4]->{'msg'}) {
#			## lookup msgid
#			$issue->[3] = $issue->[4]->{'msg'};
...
#		elsif ($issue->[4]->{'msgid'}) {
#			## lookup msgid
#			$issue->[3] = $SREF->msgs()->get($issue->[4]->{'msgid'});
#			}
#
#		if ($issue->[3] eq '') {
#			## never return a blank message, so we try and create one.
#			$issue->[3] = "$issue->[0]:$issue->[1] issue:$issue->[2] (please correct)";
#			}
#		}
#
#
...
#	#if ($params{'fast'}) {}	## we skip google notifications when doing a fast save.	
#	if ( 
#		($self->__GET__('flow/payment_method') eq 'GOOGLE') ||
#		($self->__GET__('flow/payment_method') eq 'MIXED') 
#		) {
#		## GOOGLE GETS NOTIFIED WHEN ORDERS ARE IN PENDING/APPROVED STATUS
#		my $pool = $self->__GET__('flow/pool');
#		require ZPAY::GOOGLE;
#		foreach my $payrec (@{$self->payments()}) {
#			if ($payrec->{'puuid'} ne '') {
...
#				if ($self->__GET__('flow/google_archived_ts')) {
#					if (($pool ne 'ARCHIVE') && ($pool ne 'COMPLETED')) {
#						&ZPAY::GOOGLE::unarchiveOrder($self,$payrec,0);				
#						}
#					}
#				elsif (($pool eq 'ARCHIVE') || ($pool eq 'COMPLETED')) {
#					&ZPAY::GOOGLE::archiveOrder($self,$payrec,0);
#					}
#
#				if ($self->__GET__('flow/google_processed_ts')) {}			## we've already done this
#				elsif (($pool eq 'APPROVED') || ($pool eq 'PENDING')) {
#					&ZPAY::GOOGLE::processOrder($self,$payrec,0);
#					}			
#				}
#			}
#		}
