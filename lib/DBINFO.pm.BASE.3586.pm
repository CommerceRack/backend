...
%DBINFO::CREDENTIALS = (
	'BACKUP'=>	["ORDERS","spif35k6k1**3","DBI:mysql:database=ORDERS;host=192.168.2.31"],
	'SNAP'=>		['dbuser',"g00b33RRzz",	  "DBI:mysql:database=SNAP;host=192.168.2.34;net_read_timeout=60"],
	# 'CRACKLE'=>	["ORDERS","spif35k6k1**3","DBI:mysql:database=ORDERS;host=192.168.2.35;net_read_timeout=60"],
	'CRACKLE'=>	["ORDERS","asdf","DBI:mysql:database=ORDERS;host=192.168.2.35;net_read_timeout=60"],
	# 'POP'=>		["ORDERS","spif35k6k1**3","DBI:mysql:database=ORDERS;host=192.168.2.36;net_read_timeout=60"],
	'POP'=>		["ORDERS","spif35k6k1**3","DBI:mysql:database=ORDERS;host=192.168.2.40;net_read_timeout=60"],
	'ENDOR'=>		["ORDERS","spif35k6k1**3","DBI:mysql:database=ORDERS;host=192.168.2.40;net_read_timeout=60"],
	'DAGOBAH'=>	["dbuser","io45uo23/3IIX","DBI:mysql:database=ORDERS;host=192.168.2.37;net_read_timeout=60"],
	'HOTH'=>		['dbuser',"g00b33RRzz",	  "DBI:mysql:database=SNAP;host=192.168.2.38;net_read_timeout=60"],
	'BESPIN'=>	['dbuser',"g00b33RRzz",	  "DBI:mysql:database=SNAP;host=192.168.2.39;net_read_timeout=60"],
	# 'BESPIN'=>	['dbuser',"g00b33RRzz",	  "DBI:mysql:database=SNAP;host=127.0.0.1:3307;net_read_timeout=60"],
	);
...
sub fetch_all_into_hashref {
	...
	if ( (lc($cluster) eq '_self_') || ($cluster eq uc(&ZOOVY::resolve_cluster($cluster))) ) {
		## run on all users
...
sub db_user_connect {
	...
	if ($DBINFO::HAS_SOCKET) {
	...
	elsif (substr($USERNAME,0,1) eq '@') {
		## you can pass @BEAST, @SNAP to tell us to implicitly connect to a cluster and return a handle for it.
		warn("no longer permitted to connect by cluster");
		$CLUSTER = uc(substr($USERNAME,1));
		$USERNAME = "$CLUSTER";	# we create a user on each cluster, that is the name of the cluster.
		}
	else {
		#my $PATH = &ZOOVY::resolve_userpath($USERNAME);
		# if (&ZOOVY::servername() eq 'dev') { $PATH =~ s/endor/pop/; }

		#if (-f "$PATH/dbpasswd") {
		#	my $DBNAME = uc($USERNAME);
		#	my $CLUSTER = &ZOOVY::resolve_cluster($USERNAME);
		#	# if ((&ZOOVY::servername() eq 'dev') && ($CLUSTER eq 'endor')) { $CLUSTER = 'pop'; }
		#	open F, "</$PATH/dbpasswd"; my $DBPASS = <F>; $DBPASS =~ s/\n//gs; close F;
		#	print STDERR "GOT DBPASS: $DBPASS\n";
		#	$DBINFO::CREDENTIALS{uc($USERNAME)} = [ lc($USERNAME), $DBPASS, "DBI:mysql:database=$DBNAME;host=$CLUSTER;net_read_timeout=60" ];
		#	}
		#else {
		#	my ($gref) = &ZWEBSITE::fetch_globalref($USERNAME);
		#	if ($gref->{'dbuser'} && $gref->{'dbpass'} && $gref->{'dbdsn'}) {
		#		$DBINFO::CREDENTIALS{uc($USERNAME)} = [ $gref->{'dbuser'}, $gref->{'dbpass'}, $gref->{'dbdsn'} ];
		#		$CLUSTER = uc($USERNAME);
		#		}	
		#	else {
		...
		#		}
		#	}
		my ($dbuser,$dbpass,$dbdsn) = &ZOOVY::resolve_user_db($USERNAME);
		if ($dbuser && $dbpass && $dbdsn) {
			$DBINFO::CREDENTIALS{uc($USERNAME)} = [ $dbuser, $dbpass, $dbdsn ];
			}
		else {
			($CLUSTER) = uc(&ZOOVY::resolve_cluster($USERNAME));
			}
	...
	elsif (not defined $DBINFO::USER_HANDLES{"$CLUSTER"}) {
		## create a new USER_HANDLES object
		$DBINFO::USER_HANDLES{$CLUSTER} = $HANDLEREF;
		}
	elsif (not defined $DBINFO::USER_HANDLES{"$CLUSTER"}->[0]) {
		## create a new USER_HANDLES object
		$DBINFO::USER_HANDLES{$CLUSTER} = $HANDLEREF;
		}
	elsif ($DBINFO::USER_HANDLES{"$CLUSTER"}->[1] == 0) {
		## cached counts should NEVER be at zero, this is a cause to reset the whole thing!		
		$DBINFO::USER_HANDLES{$CLUSTER} = $HANDLEREF;
		}
	elsif (defined $DBINFO::USER_HANDLES{"$CLUSTER"}) {
		## hurray, we have that instance already!
		$HANDLEREF = $DBINFO::USER_HANDLES{"$CLUSTER"};
	...
	## SANITY: if we made it here, we're creating a handle	
	#if (&ZOOVY::servername() eq 'dev') {
	#	print STDERR Dumper(\%DBINFO::CREDENTIALS);
	#	}

	my ($dbuser,$dbpass,$dbdsn,$dbname,$dbhost) = (undef,undef,undef,undef,undef);
	if ($DBINFO::HAS_SOCKET) {
		#print STDERR "HAS_SOCKET [$USERNAME]\n";
		
		if ( (lc($USERNAME) eq '_self_') || (lc($USERNAME) eq lc(&ZOOVY::servername())) ) {
			($dbuser,$dbpass,$dbdsn) = (undef,undef,'DBI:mysql:mysql_socket=/local/tmp/mysql.sock');
			}
		else {
			## we're targeting a specific user.
			## print "USERNAME: $USERNAME\n";
			($dbuser,$dbpass,$dbdsn,$dbname,$dbhost) = &ZOOVY::resolve_user_db($USERNAME);
			if ($dbhost eq lc(&ZOOVY::servername())) {
				## the dbhost is this host. (don't change this, because accessing
				## ex: crackle at it's host address by itself won't work
				$dbdsn = 'DBI:mysql:mysql_socket=/local/tmp/mysql.sock';
				}
			if (not $dbuser || not $dbpass || not $dbdsn) {
				## using legacy lookup route
				warn "resolve user_db had miss...\n";
				$CLUSTER = &ZOOVY::resolve_cluster($USERNAME);
				($dbuser,$dbpass,$dbdsn) = @{$DBINFO::CREDENTIALS{uc($CLUSTER)}};
				## ($dbuser,$dbpass,$dbdsn) = (undef,undef,'DBI:mysql:mysql_socket=/local/tmp/mysql.sock');
				}
			##print "$dbuser,$dbpass,$dbdsn,$dbname,$dbhost\n";
			}
		}
	elsif (defined $DBINFO::CREDENTIALS{uc($USERNAME)}) {
		## user specific db
		($dbuser,$dbpass,$dbdsn) = @{$DBINFO::CREDENTIALS{uc($USERNAME)}};
		}
	else {
		($dbuser,$dbpass,$dbdsn) = @{$DBINFO::CREDENTIALS{uc($CLUSTER)}};
		}
