...
sub VerifyConfig {
	...
	my %vars = ();
	$vars{'ebay:reserve_price'} = 0;
	$vars{'ebay:start_price'} = 1.00;
	$vars{'ebay:qty'} = 1;
	$vars{'ebay:duration'} = 7;
	$vars{'ebay:title'} = "Test Title";
	...
	require PRODUCT;
	my ($P) = PRODUCT->new($self->username(),'','tmp'=>1,'%prodref'=>\%vars);
	...
	my ($le) = LISTING::EVENT->new(USERNAME=>$USERNAME,
		...
		SKU=>'**DUMMY**',
		TARGET=>'EBAYA',
		PRT=>$self->prt(),VERB=>'PREVIEW');
	require LISTING::EBAY;
	my ($udbh) = &DBINFO::db_user_connect($USERNAME);
	my ($RESULT,$METAREF) = LISTING::EBAY::event_handler($udbh,$le,$P);
	&DBINFO::db_user_close();
	
#	my ($err,$hashref,$msgs) = EBAY::CREATE::doit2($listing); 
...
#	my ($r) =  &EBAY2::doRequest($USERNAME,$PROFILE,'VerifyAddItem',$hashref,preservekeys=>['Item'],noflatten=>['Fees'],xml=>1);
#	my ($resultref) = &EBAY::CREATE::parse_errors($r,0,$hashref,\%vars);
#	use Data::Dumper;
#	if ($options{'dumper'}) { print Dumper($hashref,$err,$resultref,$msgs,$r); }

	my $msgs = $METAREF->{'@MSGS'};
	...
	foreach my $msg (@{$msgs}) {
		...
		next if ($type eq 'DEBUG');
		$msg =~ s/[\<\>]+//gs;
		$msg = &ZOOVY::incode($msg);
		$errmsg .= "<tr><td valign=top>Z-$type</td><td valign=top>$msg</td></tr>";
		}
