   ...
   while ( my ($orderid,$status) = $sth->fetchrow() ) {
	...
	foreach my $orderid (@orders) {
		...
		foreach my $item (@{$O2->stuff2()->items()}) {
			...
			## NOTE: assemblies really fuck this shit up (since they have no price/cost)
			if ($stid =~ /\@/) { $pid = $stid; }
			...
			# print "QTY: $qty [$stid]\n";
			if ($qty>0) {
				$r->{'@BODY'}->[$element]->[5] += $qty;
				}

			$r->{'@BODY'}->[$element]->[6] += ($qty * $item->{'cost'});
			# $r->{'@BODY'}->[$element]->[6] = sprintf("%.2f",$r->{'@BODY'}->[$element]->[6]);

			$r->{'@BODY'}->[$element]->[7] += ($qty * $item->{'price'});
			# $r->{'@BODY'}->[$element]->[7] = sprintf("%.2f",$r->{'@BODY'}->[$element]->[7]);			

			if ($r->{'@BODY'}->[$element]->[5] > 0) {
				## compute SKU Price (average)
				$r->{'@BODY'}->[$element]->[4] = sprintf("%.2f",$r->{'@BODY'}->[$element]->[7] / $r->{'@BODY'}->[$element]->[5]);
				}

			#if ($stid eq 'NTB-DAYLIGHTCD') {
			#	print STDERR "element=$element value=$value oid=$orderid price=$r->{'@BODY'}->[$element]->[4] qty=$qty gqty=".$r->{'@BODY'}->[$element]->[5]." cogs=$r->{'@BODY'}->[$element]->[6] gms=$r->{'@BODY'}->[$element]->[7]\n";
			#	print Dumper(\%pidsmap);
			#	}
