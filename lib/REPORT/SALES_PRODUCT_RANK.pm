package REPORT::SALES_PRODUCT_RANK;
...
sub init {
	...
	$r->{'@HEAD'} = [
		{ id=>0, 'name'=>'Product', type=>'CHR', link=>'https://www.zoovy.com/biz/product/edit.cgi?PID=', target=>'_blank' },
		{ id=>1, 'name'=>'Product Name', type=>'CHR' },
		{ id=>1, 'name'=>'Units Sold', type=>'NUM', },
		{ id=>2, 'name'=>'Dollars Sold', type=>'NUM', sprintf=>'%.2f' },
		{ id=>2, 'name'=>'Order Count', type=>'NUM', },
...
sub work {
   ...
   while ( my ($orderid,$status) = $sth->fetchrow() ) {
	...
	my %PID_TO_ROW_MAP = ();	# contains a map of PID to row #
	...
	foreach my $orderid (@orders) {
		...
		foreach my $item (@{$O2->stuff2()->items()}) {
			my $PID = $item->{'product'};
			if ($PID eq '') { $PID = $item->{'sku'}; }

			if (defined $PID_TO_ROW_MAP{$PID}) {
				## row already exists
				my $ROW = $PID_TO_ROW_MAP{$PID};
				$r->{'@BODY'}->[ $ROW ]->[2] += $item->{'qty'};
				$r->{'@BODY'}->[ $ROW ]->[3] = sprintf("%.2f",$item->{'extended'} + $r->{'@BODY'}->[ $ROW ]->[3]);
				$r->{'@BODY'}->[ $ROW ]->[4]++;
				}
			else {
				## doesn't exist
				my @ROW = ( $PID, $item->{'prod_name'}, $item->{'qty'}, sprintf("%.2f",$item->{'extended'}), 1 );
				$PID_TO_ROW_MAP{$PID} = scalar(@{$r->{'@BODY'}});	# record row #
				push @{$r->{'@BODY'}}, \@ROW;
				}
