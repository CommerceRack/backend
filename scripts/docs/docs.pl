...
opendir $D, "./documentation";
while ( my $file = readdir($D) ) {
	next if (substr($file,0,1) eq '.');
	
	open F, "<./documentation/$file"; $/ = undef; my $wiki = <F>; $/ = "\n"; close F;
	$file =~ s/\.html$//gs;
	$file =~ s/_/ /gs;
	print "FILE:$file\n";
	$file =~ s/zoovy/commercerack/gs;
	...
	$wiki =~ s/zoovy/CommerceRack/igs;
	$wiki =~ s/\<aside class=\"hint\"\>[\n]*(.*?)[\n]*\<\/aside\>/{{hint|$1}}/gs;
	$wiki =~ s/\<aside class=\"caution\"\>[\n]*(.*?)[\n]*\<\/aside\>/{{caution|$1}}/gs;
	$wiki =~ s/\<aside class=\"cbc\"\>[\n]*(.*?)[\n]*\<\/aside\>/$1/gs;
	$wiki =~ s/\<li\>/# /gs;
	$wiki =~ s/<\/li>//gs;
...
#		}
	...
	my %edit = ( 'action'=>'edit', 'title'=>$file, 'text'=>$wiki );
	$mw->edit( \%edit );
	}
closedir $D;
